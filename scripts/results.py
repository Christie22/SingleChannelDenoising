...
def results(model_source, dataset_path, 
    ...
    if 'ds1' in dataset_path:
        ...
        dsname = 'ds2'
        rwnoises = [
            get_rwnoises(stationary=True, narrowband=True)[0],      # seen
            get_rwnoises(stationary=True, narrowband=True)[-1],     # unseen
            get_rwnoises(stationary=True, narrowband=False)[0],     # seen
            get_rwnoises(stationary=True, narrowband=False)[-1]     # unseen
        ]
        # noising functions
        noise_funcs = [
        ...
        rwnoises = [
            ...
            get_rwnoises(stationary=False, narrowband=True)[0],
            get_rwnoises(stationary=False, narrowband=False)[0],
            # unseen
            get_rwnoises(stationary=True, narrowband=True)[-1],
            get_rwnoises(stationary=True, narrowband=False)[-1],
            get_rwnoises(stationary=False, narrowband=True)[-1],
            get_rwnoises(stationary=False, narrowband=False)[-1]
        ]
        # noising functions
        noise_funcs = [
            *[take_file_as_noise(**rwnoise_args) for rwnoise_args in rwnoises]
